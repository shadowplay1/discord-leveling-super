...
bot.on('message', async message => {
    ...
    if (message.content.startsWith(prefix + 'rank') || message.content.startsWith(prefix + 'ranks')) {
        ...
        const member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(x => x.user.username.toLowerCase() == args.join(' ').toLocaleLowerCase()) || message.guild.members.cache.find(x => x.displayName.toLowerCase() == args.join(' ').toLocaleLowerCase()) || message.author)
        const { level, xp, maxXP, totalXP, difference } = leveling.rank(member.id, message.guild.id)
        return message.channel.send(`**${member.user.tag}**\n**------------------------**\nLevel: **${level || 1}**\nXP: **${xp || 0}/${maxXP || 300}**\nTotal XP: **${totalXP || 0}**\nXP until the next level: **${difference || 300}**`)
    }
    if (message.content.startsWith(prefix + 'leaderboard') || message.content.startsWith(prefix + 'lb')) {
        const ranks = leveling.leaderboard(message.guild.id)
        return message.channel.send(ranks.map((x, i) => `**${i + 1}** - **${x.user.tag}** - Level **${x.level}**, **${x.xp}** XP`).join('\n'))
    }
})
